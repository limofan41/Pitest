 ...
 * 
 */
public enum CRCRMutator6 implements MethodMutatorFactory {
    
  CRCR_MUTATOR6;

  private final class CRCRVisitor6 extends MethodVisitor {
    private final MutationContext context;

    CRCRVisitor6(final MutationContext context,
    ...
    private void mutate(final Double constant) {
      ...
      if (constant != 0D && constant != -1D && constant != 1D && shouldMutate(constant, replacement)) {
    ...
    private void mutate(final Float constant) {
      ...
      if (constant != 0F && constant != -1F && constant != 1F && shouldMutate(constant, replacement)) {
        translateToByteCode(replacement);
      } else {
        translateToByteCode(constant);
      }
    ...
    private void mutate(final Integer constant) {
      final Integer replacement = -constant;
      
      if (constant != 0 && constant != -1 && constant != 1 && shouldMutate(constant, replacement)) {
        translateToByteCode(replacement);
      } else {
        translateToByteCode(constant);
      }
    ...
    private void mutate(final Long constant) {
      ...
      final Long replacement = -constant;

      if (constant != 0L && constant != -1L && constant != 1L && shouldMutate(constant, replacement)) {
        translateToByteCode(replacement);
      } else {
        translateToByteCode(constant);
      }
    ...
    private <T extends Number> boolean shouldMutate(final T constant,
      ...
      final MutationIdentifier mutationId = this.context.registerMutation(
          CRCRMutator6.this, "Substituted " + constant + " with "
              + replacement);
  ...
  public MethodVisitor create(final MutationContext context,
    ...
    return new CRCRVisitor6(context, methodVisitor);
  }
