...
public enum CRCRMutator2 implements MethodMutatorFactory {
  ...
  private final class CRCRVisitor2 extends MethodVisitor {
    ...
    CRCRVisitor2(final MutationContext context,
    ...
    private void mutate(final Integer constant) {
      ...
      if (constant.intValue() != -1 && shouldMutate(constant, replacement)) {
        translateToByteCode(replacement);
      } else {
        translateToByteCode(constant);
      }
    ...
    private void mutate(final Long constant) {
      ...
      final Long replacement = (constant == -1L) ? 2L : -1L;
    ...
    private <T extends Number> boolean shouldMutate(final T constant,
      ...
      final MutationIdentifier mutationId = this.context.registerMutation(
          CRCRMutator2.this, "Substituted " + constant + " with "
              + replacement);
  ...
  public MethodVisitor create(final MutationContext context,
    ...
    return new CRCRVisitor2(context, methodVisitor);
  }
